...
# aural2 (DL, audio)
aural2_cntnr_img="openhorizon/aarch64-tx2-aural2-intu:test"
aural2_cntnr_name="aural2-intu"

# intu/self (embodiment)
intu_cntnr_img="openhorizon/cogwerx-aarch64-tx2-self:edge"
...
start() {
   ...
   echo "Starting container $face_cntnr_name"
   script_and_args="face_emotion.py -intu camera 6"
   if $display; then 
      export DISPLAY=:0   
      #script_and_args="video_emotion_color_demo.py"
...
if [[ $display == *"disp"* ]]; then
   display=true  # display checked as true/false from now on
fi
exit 0
